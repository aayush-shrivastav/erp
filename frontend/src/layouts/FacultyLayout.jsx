...
import {
    ...
    LogOut, Menu, X, Landmark, ClipboardCheck, Edit3, Megaphone
} from 'lucide-react';
...
const FacultyLayout = () => {
    ...
    const navigation = [
        ...
        { name: 'Take Attendance', href: '/faculty/attendance', icon: ClipboardCheck },
        { name: 'Enter Marks', href: '/faculty/marks', icon: Edit3 },
        { name: 'Notice Board', href: '/faculty/notices', icon: Megaphone },
    ];
        ...
        <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
            ...
            <aside className={`fixed inset-y-0 left-0 bg-white shadow-xl z-50 w-64 transform transition-transform duration-300 ease-in-out md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:static md:flex-shrink-0 flex flex-col`}>
                <div className="h-16 flex items-center px-6 border-b border-slate-100/60 bg-white">
                    ...
                    <span className="text-xl font-black text-slate-800 tracking-tight">EduERP <span className="text-sm font-medium text-slate-400">Faculty</span></span>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
                            <div className="hidden sm:block text-right">
                                <div className="text-sm font-bold text-slate-800">{user?.email?.split('@')[0] || 'Faculty'}</div>
                                <div className="text-xs font-medium text-slate-500">Faculty Portal</div>
                            </div>
